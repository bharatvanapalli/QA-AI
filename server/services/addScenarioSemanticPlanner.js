...
Required compact shape:
{
  ...
  "cases": [{
    ...
    "actions": [{
      ...
      "expected": "optional concise expected page/control effect after this action"
    }],
    "assertions": [{
...
- When the source states an expected effect for a state-changing action, preserve it in action.expected. A following WaitForState may express that effect; do not invent a website-specific outcome.
...
const INTENT_ACTION_FIELDS = new Set([
  'type', 'sourceQuote', 'target', 'value', 'valueRef', 'selection', 'condition', 'expected',
]);
