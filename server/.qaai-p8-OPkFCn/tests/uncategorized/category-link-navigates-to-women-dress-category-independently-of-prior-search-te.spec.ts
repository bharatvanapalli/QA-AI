      ...
      const el4 = await resolveLocator(page, [
    {
      ...
      await assertTextPresent(page, "No products found", '', 10000).catch((_atp) => {
        test.info().annotations.push({ type: 'qaai-soft-fail', description: `UI_TEXT: ${_atp.message}` });
            ...
            throw new Error("QAAI parity: original browser run failed (browser run recorded a failed verdict). Generated script must not pass until the failing assertion is repaired or recaptured.");
