...
function qaaiEscapeRegex(value: string): string {
  return String(value || '').replace(/[-/\^$*+?.()|[]{}]/g, '\$&');
}

function productLikeLocator(page: Page): Locator {
...
export async function assertProductNamesContain(page: Page, expected: string, timeoutMs: number): Promise<void> {
  const safe = qaaiEscapeRegex(expected);
  const namePattern = new RegExp(safe, 'i');
...
export async function assertProductCategory(page: Page, expected: string, timeoutMs: number): Promise<void> {
  const safe = qaaiEscapeRegex(expected);
  const categoryPattern = new RegExp(safe, 'i');
...
export async function assertPricesBetween(page: Page, min: number, max: number, timeoutMs: number): Promise<void> {
