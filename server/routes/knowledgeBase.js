...
// Best-effort scan of playwright/pages/**/*.ts to find Page Object files
// that mention each locator's `element` text. Used to render the "Used by"
// footprint on each KB row so users can navigate to the actual POM file
// that embeds the locator. Cached for the lifetime of the request — the
// scan is O(pages count) which is small (single-digit folders).
function scanPageObjectFiles() {
  const out = []; // [{ relPath, name, content }]
  ...
  while (stack.length) {
    ...
    for (const e of entries) {
      ...
      try {
        ...
        const rel = path.relative(PLAYWRIGHT_DIR, full).replace(/\\/g, '/');
        out.push({ relPath: rel, name: e.name, content });
      } catch (_) {}
    }
  }
  return out;
...
// Given a locator and the cached PO file list, return the relative paths
// that reference this locator's element name or accessibleName. Heuristic
// but reliable enough for the UI footprint (matches the casing-insensitive
// element label OR accessibleName string in any string literal).
function findUsedBy(locator, pageFiles) {
  const tokens = [locator.element, locator.accessibleName]
    .filter(Boolean)
...
router.get('/', async (req, res, next) => {
  try {
    ...
    // Enrich each row with the Page Object files that reference it. The
    // scan runs once per request — `pages/` is small. The match heuristic
    // is intentionally loose so it works whether the PO file was hand-
    // written or AI-generated by the pomEmitter.
    const pageFiles = scanPageObjectFiles();
    const enriched = locators.map((l) => ({
      ...l,
      // Display-safe selector: legacy rows wrote the literal '(captured)'
      // placeholder. Surface a clearly-labelled null instead so the UI
      // can show "Not yet captured" rather than rendering placeholder text
      // as if it were code. Live rows from current Conductor always have
      // a real Playwright expression here.
      selector: (l.selector && l.selector !== '(captured)') ? l.selector : null,
      usedBy: findUsedBy(l, pageFiles),
    }));

    // Tiny health summary for the page header so we don't make the client
    // recompute it every render.
    const stats = {
      total: enriched.length,
      healthy: enriched.filter((l) => (l.healthScore ?? 100) >= 70).length,
      watching: enriched.filter((l) => {
        const s = l.healthScore ?? 100;
        return s >= 30 && s < 70;
      }).length,
      quarantined: enriched.filter((l) => (l.healthScore ?? 100) < 30).length,
    };
    res.json({ success: true, locators: enriched, stats });
  } catch (err) {
    next(err);
  }
});
...
router.post('/', requireCsrf, async (req, res, next) => {
  try {
    ...
    // P3-4: KB unique key is now (projectId, element, pageUrl). The KB
    // manual-edit UI doesn't carry pageUrl, so target the existing
    // healthiest row and create with pageUrl='' when nothing exists yet.
    const existing = await prisma.knowledgeBaseLocator.findFirst({
      where: { projectId: project.id, element },
