...
function supportedFormatsFor(context) {
  return context === 'test-data' ? TEST_DATA_FORMATS : DOCUMENT_FORMATS;
}

function sanitizeBody(body, { context = 'general' } = {}) {
  const text = String(body == null ? '' : body).replace(/\s+/g, ' ').trim();
  const formats = supportedFormatsFor(context);
  if (!text) return 'Something went wrong. Please try again.';

  if (/prisma|p20\d{2}|unique constraint|foreign key|constraint failed|database|sqlite|postgres|sql\b|queryraw/i.test(text)) {
    return 'The request could not be saved. Please retry; if it continues, contact support.';
  }

  if (/docx parse failed|mammoth|could not find main document part|valid \.docx|end of central directory|corrupt(ed)? zip|zip file/i.test(text)) {
  ...
  if (/cannot read properties of (null|undefined)|cannot destructure property|is not iterable|is not a function|typeerror|referenceerror|syntaxerror/i.test(text)) {
...
export function sanitizeUiMessage(message, opts = {}) {
