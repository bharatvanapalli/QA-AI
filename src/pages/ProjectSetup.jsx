    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto bg-[radial-gradient(circle_at_10%_0%,rgba(59,130,246,0.10),transparent_30%),radial-gradient(circle_at_92%_12%,rgba(16,185,129,0.10),transparent_26%),linear-gradient(180deg,#f8fafc,#eef3f8)]">
        <div className="max-w-[1500px] mx-auto px-6 py-7">
          <div className="grid grid-cols-1 xl:grid-cols-[310px_minmax(0,1fr)] gap-5 items-start">
...
function ProjectCard({ project, isCurrent, onActivate, onChanged }) {
  ...
  const counts = project._count || {};
  const setupTabs = [
    { id: 'core', label: 'Core', icon: Settings, hint: 'Identity and execution', metric: f.isDirty ? 'Unsaved' : 'Ready' },
    { id: 'users', label: 'Users', icon: Users, hint: 'Login accounts', metric: 'Auth' },
    { id: 'sessions', label: 'Sessions', icon: KeyRound, hint: 'Saved browser state', metric: 'Fixtures' },
    { id: 'atlas', label: 'Site atlas', icon: ScanLine, hint: 'Live structure map', metric: 'Crawl' },
    { id: 'repo', label: 'Git', icon: GitBranch, hint: 'Source workspace', metric: 'Repo' },
    { id: 'browser', label: 'Browser', icon: ShieldCheck, hint: 'Runtime context', metric: 'MCP' },
    { id: 'popups', label: 'Popups', icon: Sparkles, hint: 'Known overlays', metric: 'Rules' },
    { id: 'sprints', label: 'Sprints', icon: Calendar, hint: 'Release cycles', metric: 'Cycles' },
  ];
  ...
  return (
    <div className={`overflow-hidden rounded-[34px] border bg-[linear-gradient(135deg,rgba(236,253,245,0.72),rgba(255,255,255,0.84)_38%,rgba(239,246,255,0.74)_74%,rgba(245,243,255,0.76))] shadow-[0_30px_80px_-48px_rgba(15,23,42,0.48),0_1px_0_rgba(255,255,255,0.9)_inset] backdrop-blur-xl ${isCurrent ? 'border-ink-300/70' : 'border-white/75'}`}>
      <div className="relative overflow-hidden border-b border-white/70 px-6 py-6">
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_7%_8%,rgba(16,185,129,0.20),transparent_30%),radial-gradient(circle_at_92%_18%,rgba(59,130,246,0.16),transparent_36%),linear-gradient(180deg,rgba(255,255,255,0.58),rgba(255,255,255,0.18))]" aria-hidden="true" />
        <div className="relative">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            <div className="flex items-start gap-4">
              <span className="inline-flex h-14 w-14 shrink-0 items-center justify-center rounded-[20px] bg-ink-900 text-white shadow-[0_20px_44px_-26px_rgba(15,23,42,0.92)]">
                <Folder className="w-6 h-6" aria-hidden="true" />
              </span>
              <div className="min-w-0">
                <div className="flex items-center gap-2 min-w-0">
                  <h3 className="text-3xl font-black text-ink-950 tracking-tight truncate">{project.name}</h3>
                  {isCurrent && (
                    <span className="shrink-0 text-xs uppercase tracking-wider font-bold text-success-700 bg-success-50/90 px-3 py-1.5 rounded-full border border-success-200/80 shadow-sm">
                      Active
                    </span>
                  )}
                </div>
                <p className="text-base text-ink-500 mt-1 truncate">
        ...
        <div className="qaai-scrollbar-none -mx-1 mt-5 flex gap-2 overflow-x-auto px-1 pb-1" role="tablist" aria-label="Project setup sections">
          {setupTabs.map((tab) => (
            <ProjectSetupTab
              key={tab.id}
              tab={tab}
      ...
      <div className="bg-[linear-gradient(180deg,rgba(248,250,252,0.58),rgba(236,242,248,0.58))] p-5 sm:p-6">
        <section className={`rounded-[28px] border border-white/75 bg-white/74 p-4 shadow-[0_22px_55px_-40px_rgba(15,23,42,0.48),0_1px_0_rgba(255,255,255,0.92)_inset] backdrop-blur-xl ${activePanel === 'core' ? 'block' : 'hidden'}`}>
          <div className="flex items-center justify-between gap-3 mb-4">
            <div>
              <h4 className="text-lg font-black text-ink-950">Core settings</h4>
              <p className="text-sm text-ink-500 mt-0.5">These values drive generation, execution, and exported frameworks.</p>
            </div>
            {f.isDirty && (
        ...
        <section className={`${activePanel === 'core' ? 'hidden' : 'block'} rounded-[28px] border border-white/75 bg-white/74 p-4 shadow-[0_22px_55px_-40px_rgba(15,23,42,0.48),0_1px_0_rgba(255,255,255,0.92)_inset] backdrop-blur-xl`}>
          <div className="flex items-center justify-between gap-3">
            <div>
              <h4 className="text-lg font-black text-ink-950">{setupTabs.find((tab) => tab.id === activePanel)?.label}</h4>
              <p className="text-sm text-ink-500 mt-0.5">{setupTabs.find((tab) => tab.id === activePanel)?.hint}</p>
            </div>
          </div>

          <div className="mt-4">
            {activePanel === 'users' && <TestUsersEditor projectId={project.id} />}
            {activePanel === 'sessions' && <AuthFixturesPanel projectId={project.id} />}
            {activePanel === 'atlas' && <CalibratorPanel projectId={project.id} />}
            {activePanel === 'repo' && <GitRepoEditor projectId={project.id} />}
            {activePanel === 'browser' && <BrowserContextEditor projectId={project.id} />}
            {activePanel === 'popups' && <KnownPopupsEditor projectId={project.id} />}
            {activePanel === 'sprints' && <SprintsEditor projectId={project.id} isCurrent={isCurrent} />}
          </div>
        </section>
      </div>
      <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-white/70 bg-white/62 backdrop-blur">
...
function ProjectStat({ label, value }) {
  return (
    <div className="rounded-2xl border border-white/75 bg-white/76 px-4 py-3 shadow-[0_12px_28px_-22px_rgba(15,23,42,0.42),0_1px_0_rgba(255,255,255,0.86)_inset] transition-all duration-200 hover:-translate-y-0.5 hover:bg-white/92 hover:shadow-[0_18px_34px_-24px_rgba(15,23,42,0.48)]">
...
function ProjectSetupTab({ tab, active, onClick }) {
  const Icon = tab.icon;
  return (
    <button
      type="button"
      ...
      onClick={onClick}
      className={`group/setup-tab flex min-w-[150px] items-center gap-3 rounded-2xl border px-3 py-3 text-left transition-all duration-200 focus-visible:outline-none focus-visible:shadow-ring ${
        active
          ? 'border-ink-900 bg-ink-900 text-white shadow-[0_18px_38px_-24px_rgba(15,23,42,0.82)]'
          : 'border-white/75 bg-white/66 text-ink-700 shadow-sm hover:-translate-y-0.5 hover:bg-white/92 hover:shadow-[0_16px_30px_-24px_rgba(15,23,42,0.48)]'
      }`}
    >
      <span className={`inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl transition-colors ${
        active ? 'bg-white/15 text-white' : 'bg-ink-900 text-white group-hover/setup-tab:bg-ink-800'
      }`}>
        <Icon className="w-4 h-4" aria-hidden="true" />
      </span>
      <span className="min-w-0">
        <span className="block truncate text-sm font-bold">{tab.label}</span>
        <span className={`mt-0.5 block truncate text-[11px] ${active ? 'text-white/62' : 'text-ink-500'}`}>
          {tab.metric}
        </span>
      </span>
    </button>
