...
// ─────────────────────────────────────────────────────────────────────────────
function AddScenarioModal({ open, onClose, onSubmit, submitting, scenarios = [] }) {
  ...
  const canSubmit = design.trim().length > 8 && (!continueFromCase || !!selectedContinuation) && !submitting;
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      <motion.div
        ...
        className="w-full max-w-xl rounded-[22px] border border-white/70 bg-white/95 backdrop-blur-xl shadow-card p-5"
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <h2 className="text-lg font-bold text-ink-950">Add a scenario</h2>
            <p className="mt-0.5 text-sm text-ink-500">
              Describe the test you want. QAAI authors it grounded in the verified site atlas + your uploaded test data, then adds it to the current suite for review.
            </p>
          </div>
        </div>

        <label className="mt-4 block text-xs font-semibold uppercase tracking-wide text-ink-500">Test design</label>
        <textarea
          value={design}
        ...
        <label className={`mt-3 flex items-start gap-2.5 select-none ${continuationOptions.length ? 'cursor-pointer' : 'cursor-not-allowed opacity-55'}`}>
          <input
            ...
            disabled={!continuationOptions.length || submitting}
            onChange={(e) => {
...
function ScenarioCard({
  ...
  activeHoverScenarioId, onHoverScenario,
}) {
...
// ─────────────────────────────────────────────────────────────────────────────
function CaseRow({ tc, index, onApprove, onReject, bulkMode, checked, onToggleBulk, stepStatuses, smokeSelected, onToggleSmoke, onRefine, onRestore, operation, hoverClearToken }) {
    ...
    >
      ...
      <AnimatePresence initial={false}>
        {displayStepsOpen && steps.length > 0 && (
          ...
          >
            <StepsPanel steps={steps} stepStatuses={stepStatuses} />
          </motion.div>
        )}
      </AnimatePresence>
    </motion.li>
...
function StepsPanel({ steps, stepStatuses, floating = false }) {
  return (
    <div className={`${floating ? 'max-h-[min(52vh,520px)] overflow-y-auto overscroll-contain rounded-xl border border-white/60 bg-white/62' : 'ml-12 mr-5 mb-4 rounded-xl border border-white/60 bg-white/50'} backdrop-blur-sm overflow-hidden`}>
      <div className="px-3 py-2 bg-white/70 border-b border-white/60 text-2xs font-bold uppercase tracking-wider text-ink-500">
        Steps
      </div>
      <ol className="divide-y divide-white/50">
        {steps.map((step, i) => (
          <StepRow
            ...
            step={step}
            index={i + 1}
            status={stepStatuses?.[i + 1]?.status || null}
            error={stepStatuses?.[i + 1]?.error || null}
            operationCheck={stepStatuses?.[i + 1]?.operationCheck || null}
            assertion={stepStatuses?.[i + 1]?.assertion || null}
          />
        ))}
      </ol>
    </div>
...
function StepRow({ step, index, status, error, operationCheck, assertion }) {
  ...
  const assertionText = assertion?.expected || (step.verificationPoint ? step.expected : null);
  const legacyExpectedText = !checkText && !assertionText ? step.expected : null;
  return (
    <li className={`grid grid-cols-[40px_1fr] gap-3 px-3 py-2.5 ${badge ? `ring-1 ring-inset ${badge.ring}` : ''}`}>
      ...
      <div className="min-w-0">
        ...
        {step.value && (
          <div className="text-xs text-ink-600 mt-0.5">
            ...
            <span className="font-mono text-ink-800 bg-white/70 px-1 rounded">"{step.value}"</span>
          </div>
        )}
  ...
  const handleApproveAddScenarioPreview = useCallback(async () => {
    ...
    try {
      ...
      toast.error(reviewPayload?.message || err?.message || 'The draft could not be approved. The reviewed draft was retained.', {
        title: reviewPayload?.code ? `Approval not applied · ${reviewPayload.code}` : 'Approval failed',
        ttl: 12000,
      });
    } finally {
