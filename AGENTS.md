# AGENTS.md — QAAI Portal constitution

This file is auto-loaded into every Codex session. Read [BUILD_PLAN.md](BUILD_PLAN.md) for phase status and [PHASE_LOG.md](PHASE_LOG.md) for what was built in prior phases.
...
Stack: React 19, Vite 6, Tailwind 3 with custom tokens, Express, Prisma 5.22, SQLite, Codex Sonnet 4.6 via `@anthropic-ai/sdk`, MCP for browser control, Playwright for execution, WebSocket per-user channel.
...
When proposing a new LLM call, write the case for it in the PR description: *what novelty does Codex provide here that a function couldn't?* If you can't answer that, the design is wrong — refactor before merging.
...
- Locator failures trigger the healer (`server/services/agents/healer.js`) BEFORE Codex sees the error. Codex continues as if the tool succeeded — less context churn, fewer tokens.
...
- Conductor MUST call the `assertion_check` synthetic tool for each declared assertion before emitting `RESULT: pass`. The tool fabricates its response from `mcp.getLastSnapshot()` — no real MCP roundtrip, no Codex call.
...
10. If you made an architectural decision worth keeping forever, update AGENTS.md.
