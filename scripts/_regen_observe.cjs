'use strict';
// Regenerate ALL scenarios with a forced atlas refresh, observing the crawl +
// architect + data-mapping events live. Polls until a new generation (version >
// current) is produced and populated.
...
const CSRF = 'regen-csrf';
const LOG = path.join(ROOT, '_regen_observe.log');
fs.writeFileSync(LOG, '');
const log = (l) => { const s = `[${new Date().toISOString()}] ${l}`; fs.appendFileSync(LOG, s + '\n'); };
function sum(ev){ if(!ev||typeof ev!=='object') return String(ev); const t=ev.type||'?'; const b=[]; for(const k of ['phase','level','status','count','total','scenarios','sheet','rows','mapped']) if(ev[k]!=null) b.push(`${k}=${typeof ev[k]==='object'?JSON.stringify(ev[k]):ev[k]}`); let m=ev.message||ev.detail||''; if(typeof m==='object') m=JSON.stringify(m); if(m&&m.length>320) m=m.slice(0,320)+'…'; return `${t} ${b.join(' ')}${m?' :: '+m:''}`; }
...
(async () => {
  ...
  log(`current generation version = ${beforeVer}`);
  ...
  ws.on('message', (raw) => { let ev; try{ev=JSON.parse(raw.toString());}catch{return;} if(ev&&ev.type==='connected') return;
    const t = ev.type || '';
    // keep architect/crawl/mapping/progress/rate-limit; drop noisy frame/snapshot
    if (/frame|snapshot.preview|download/i.test(t)) return;
    log('WS ' + sum(ev)); });
  ws.on('error', (e) => log('WS error ' + e.message));
  await new Promise((r)=>setTimeout(r,1500));

  log('POST /scenarios/generate {replace:true, forceAtlasRefresh:false (fresh atlas reused)} …');
  try { const r = await fetch(`${BASE}/api/projects/${PROJECT_ID}/scenarios/generate`, { method:'POST', headers:H, body: JSON.stringify({ replace:true, forceAtlasRefresh:false }) }); const b=await r.json().catch(()=>({})); log(`/generate → HTTP ${r.status} :: ${JSON.stringify(b).slice(0,300)}`); }
  catch(e){ log('generate err '+e.message); }

  const DEADLINE = Date.now() + 16*60*1000; let done=false;
  while (Date.now() < DEADLINE) {
    ...
    if (g && g.version > beforeVer) {
      ...
      // wait until cases are populated with steps (generation finished authoring)
      if (casesWithSteps >= scn && scn > 0) { await new Promise((r)=>setTimeout(r,4000)); log(`REGEN_DONE v${g.version} scenarios=${scn} casesWithSteps=${casesWithSteps} genId=${g.id}`); break; }
    }
  }
  if (!done) log('REGEN_TIMEOUT (no new generation)');
  try { ws.close(); } catch {}
