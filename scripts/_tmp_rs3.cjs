...
async function go() {
  ...
  const cases = await p.testCaseResult.findMany({
    where: { runId: '2fda1038-bece-43f2-add9-0a7b0817dda3' },
    select: { id: true, status: true, testCaseId: true },
    orderBy: { updatedAt: 'desc' }
  });
  console.log('total case results:', cases.length);
  cases.forEach(c => console.log(`  ${c.status} | tcId=${c.testCaseId?.slice(0,8)} | id=${c.id?.slice(0,8)}`));
  await p.$disconnect();
}
