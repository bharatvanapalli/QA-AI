...
function runAndPersist(packageDir, commands) {
  ...
  const lastRunFile = path.resolve(packageDir, 'test-results/.last-run.json');
  fs.writeFileSync(lastRunFile, lastRunContent, 'utf8');
  ...
  const exportVerFile = path.resolve(packageDir, 'EXPORT_VERIFICATION.json');
  fs.writeFileSync(exportVerFile, exportVerificationContent, 'utf8');
