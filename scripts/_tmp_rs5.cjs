const { PrismaClient } = require('../server/node_modules/@prisma/client');
const p = new PrismaClient();
async function go() {
  const cases = await p.runResult.findMany({
    where: { runId: '2fda1038-bece-43f2-add9-0a7b0817dda3' },
    select: { id: true, status: true, testCaseId: true, flipDirection: true },
    orderBy: { createdAt: 'desc' }
  });
  console.log('total runResults:', cases.length);
  const byStatus = {};
  cases.forEach(c => { byStatus[c.status] = (byStatus[c.status]||0)+1; });
  console.log('by status:', JSON.stringify(byStatus));
  console.log('--- all results ---');
  cases.forEach(c => console.log(`  ${c.status.padEnd(12)} | flip=${c.flipDirection||'none'} | tcId=${c.testCaseId?.slice(0,8)} | rrId=${c.id?.slice(0,8)}`));
  await p.$disconnect();
}
go().catch(e => { console.error(e.message); p.$disconnect(); });
